...
snr_err_bits_awgn = zeros(size(desired_snr_db,1),1);
snr_BER_awgn = zeros(size(desired_snr_db,1),1);
...
%         plot(1:N,imag(h));
        ...
        n_awgn = ( randn(N,1) + 1i*randn(N,1) )*sqrt(N0/2);
        r_awgn = s + n_awgn;
        ...
        %AWGN channel
        y_dist_awgn = zeros(N,4);
        y_dist_awgn(:,1) = abs(r_awgn-y(1)).^2;
        y_dist_awgn(:,2) = abs(r_awgn-y(2)).^2;
        y_dist_awgn(:,3) = abs(r_awgn-y(3)).^2;
        y_dist_awgn(:,4) = abs(r_awgn-y(4)).^2;

        decision_matrix_awgn = zeros(N,1);
        for k = 1:N
            min_dist = find( y_dist_awgn(k,:) == min(y_dist_awgn(k,:)) );
            decision_matrix_awgn(k) = y(min_dist(1));
        end

        error_matrix_awgn = s-decision_matrix_awgn;
        error_indices_awgn = find(error_matrix_awgn);
        exper_P_err_awgn = size( error_indices_awgn,1 )/N;
        theor_P_err_awgn = 1/(2*snr);
        %% 5
...
%         end
        ...
        %AWGN channel
        err_bits_awgn = err_bits_awgn + sum( abs( (error_matrix_awgn).^2 )/4 );  
...
%     disp(err_bits/(N*packet_num));
    ...
    snr_err_bits_awgn(snr_i) = err_bits_awgn;
%     disp(err_bits);
    snr_BER_awgn(snr_i) = err_bits_awgn/(N*packet_num);
%     disp(err_bits/(N*packet_num));
end

theoretical_BER = 1./(2*desired_snr_db);
theoretical_BER_awgn = (1/2)*exp(-desired_snr_db);

figure;
plot(desired_snr_db, snr_BER);
hold on
plot(desired_snr_db, snr_BER_awgn);
plot(desired_snr_db, theoretical_BER);
plot(desired_snr_db, theoretical_BER_awgn);
xlabel('SNR','Interpreter','latex');
ylabel('BER','Interpreter','latex');
legend("flat fading BER","AWGN BER","Theoretical flat fading BER","Theoretical AWGN BER");
legend show

figure;
semilogy(desired_snr_db, snr_BER);
hold on
semilogy(desired_snr_db, snr_BER_awgn);
semilogy(desired_snr_db, theoretical_BER);
semilogy(desired_snr_db, theoretical_BER_awgn);
xlabel('SNR','Interpreter','latex');
ylabel('BER','Interpreter','latex');
legend("flat fading BER","AWGN BER","Theoretical flat fading BER","Theoretical AWGN BER");
legend show